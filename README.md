...
- [x] 입력 값으로 쉼표(,) 또는 콜론(:)을 구분자로 가지는 문자열이 들어올 경우, 구분자를 기준으로 분리한 각 숫자의 합을 반환한다.
- [x] 커스텀 구분자를 지정할 수 있다.
  - 커스텀 구분자는 문자열 앞부분의 "//"와 "\n" 사이에 위치하는 문자를 사용한다.
  - 예: "//;\n1;2;3"의 경우 구분자는 세미콜론(;)이다.
- [x] 사용자가 잘못된 값을 입력할 경우 `IllegalArgumentException`을 발생시킨다.
  - 음수를 입력하는 경우 예외를 발생시킨다.
  - 숫자가 아닌 값을 입력하는 경우 예외를 발생시킨다.
- [x] 사용자의 입력을 받아 계산 결과를 출력한다.
  - "덧셈할 문자열을 입력해 주세요." 문구를 출력하고 입력을 받는다.
